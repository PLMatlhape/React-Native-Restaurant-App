...
const OrderManagementScreen: React.FC = () => {
  ...
  const loadOrders = async (): Promise<void> => {
    ...
    if (result.success && result.data) {
      ...
      const sortedOrders = (result.data as Order[]).sort((a, b) => 
  ...
  const renderFilterButton = (status: string, label: string): React.ReactElement => (
    <TouchableOpacity
      key={status}
      style={[
        styles.filterButton,
  ...
  const renderStatusOption = (status: string, label: string): React.ReactElement => (
    <TouchableOpacity
      key={status}
